"""
INTERNAL VALIDATION SCRIPT -- NOT the KLA benchmarking entry point.
=====================================================================
This script requires ground truth and a full data_root/ layout
(calib_stats.json, train/gt, train/NoisyLR). It is used for this team's
own model comparison and ablation work (see report / training log).

KLA's benchmarking team should use evaluate.py instead -- that script
takes a bare test-images directory with no ground truth and no
calib_stats.json dependency, matching the submission spec exactly.

Usage:
    python internal_validation.py /path/to/data_root /path/to/checkpoint.pt
"""
...
# ---------------------------------------------------------------------------
# Metrology Support (JSON Encoder + KPI Logic)
# ---------------------------------------------------------------------------
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python internal_validation.py /path/to/data_root /path/to/checkpoint.pt")
        return
