"""
Trains with synthetic degradation-diversity augmentation added on top of
the baseline config -- ONLY this changes vs. baseline (num_blocks,
edge_weight, freq_weight, weight_decay, lr all pinned to baseline's
proven values), per the ablation discipline used throughout.

The validation split is built WITHOUT synthetic augmentation (real data
only) -- so val_psnr/val_ssim here are directly comparable to baseline's
and hard-oversample's numbers, not confounded by also evaluating on
synthetic noise.

Usage:
    python train_synthetic_aug.py /path/to/data_root
"""

import os
import sys
import time

import torch

from sem_dataset import SEMPairDataset, SEMTestDataset, load_calib_stats
from synthetic_augment import make_synthetic_aug_dataset
from model import SemiRestoreNet_V2, KLAMetrologyLoss, build_optimizer
from train_engine import HardwareEngine

SEMPairDatasetSyntheticAug = make_synthetic_aug_dataset(SEMPairDataset)


def compute_psnr(pred, target, max_val=1.0):
    mse = torch.mean((pred - target) ** 2)
    if mse == 0:
        return float('inf')
    return (10 * torch.log10(max_val ** 2 / mse)).item()


def main():
    data_root = sys.argv[1] if len(sys.argv) > 1 else "./data"

    # Identical to train_baseline.py's config -- only the dataset changes.
    config = dict(
        dim=64,
        num_blocks=2,
        batch_size=4,
        target_batch_size=32,
        scale_factor=2,
        edge_weight=0.5,
        freq_weight=0.0,
        ssim_weight=0.0,
        lr=2e-4,
        weight_decay=1e-4,
        scheduler_patience=3,
        early_stop_patience=7,
        num_workers=min(8, os.cpu_count() or 1),
        num_epochs=50,
        use_compile=False,
        checkpoint_dir="./checkpoints_synthetic_aug",
        # Synthetic augmentation params
        synthetic_noise_prob=0.5,
        synthetic_noise_sigma_range=(0.02, 0.10),
        synthetic_blur_prob=0.3,
        synthetic_blur_sigma_range=(0.3, 0.8),
    )
    print("=== SYNTHETIC DEGRADATION-DIVERSITY AUGMENTATION RUN ===")
    print("Config:", config)

    if not torch.cuda.is_available():
        print("WARNING: no CUDA device found.")
        return

    os.makedirs(config["checkpoint_dir"], exist_ok=True)
    p_low, p_high = load_calib_stats(data_root)

    # ---- Train split: SYNTHETIC AUGMENTATION ON ----
    full_train_aug = SEMPairDatasetSyntheticAug(
        gt_dir=os.path.join(data_root, "train", "gt"),
        lr_dir=os.path.join(data_root, "train", "NoisyLR"),
        p_low=p_low, p_high=p_high, scale_factor=config["scale_factor"],
        augment=True,  # geometric (rot90/flip) augmentation, same as baseline
        synthetic_aug=True,
        noise_prob=config["synthetic_noise_prob"],
        noise_sigma_range=config["synthetic_noise_sigma_range"],
        blur_prob=config["synthetic_blur_prob"],
        blur_sigma_range=config["synthetic_blur_sigma_range"],
    )
    # ---- Val split: SAME underlying files, but plain SEMPairDataset
    # (no synthetic aug, no geometric aug) -- so val metrics are directly
    # comparable to every other run's val metrics. ----
    full_train_plain = SEMPairDataset(
        gt_dir=os.path.join(data_root, "train", "gt"),
        lr_dir=os.path.join(data_root, "train", "NoisyLR"),
        p_low=p_low, p_high=p_high, scale_factor=config["scale_factor"],
        augment=False,
    )

    n_val = max(1, int(len(full_train_aug) * 0.1))
    n_train = len(full_train_aug) - n_val
    generator = torch.Generator().manual_seed(42)  # SAME seed as every other script
    train_subset, val_subset = torch.utils.data.random_split(
        range(len(full_train_aug)), [n_train, n_val], generator=generator
    )
    train_indices = list(train_subset)
    val_indices = list(val_subset)

    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.Subset(full_train_aug, train_indices),
        batch_size=config["batch_size"], shuffle=True,
        num_workers=config["num_workers"], pin_memory=True, drop_last=True,
    )
    val_loader = torch.utils.data.DataLoader(
        torch.utils.data.Subset(full_train_plain, val_indices),
        batch_size=config["batch_size"], shuffle=False,
        num_workers=config["num_workers"], pin_memory=True,
    )

    model = SemiRestoreNet_V2(dim=config["dim"], num_blocks=config["num_blocks"],
                               scale_factor=config["scale_factor"])
    criterion = KLAMetrologyLoss(edge_weight=config["edge_weight"],
                                  freq_weight=config["freq_weight"],
                                  ssim_weight=config["ssim_weight"])

    engine = HardwareEngine(model, batch_size=config["batch_size"],
                             target_batch_size=config["target_batch_size"])
    criterion = criterion.to(engine.device)

    optimizer = build_optimizer(engine.model, lr=config["lr"],
                                 weight_decay=config["weight_decay"])
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=0.5, patience=config["scheduler_patience"]
    )

    best_val_loss = float('inf')
    epochs_since_improvement = 0

    for epoch in range(config["num_epochs"]):
        t0 = time.time()

        train_losses = []
        for lr_img, gt_img in train_loader:
            loss = engine.train_step(optimizer, criterion, None, lr_img, gt_img)
            train_losses.append(loss)
        avg_train_loss = sum(train_losses) / len(train_losses)

        val_losses, val_psnrs = [], []
        for lr_img, gt_img in val_loader:
            loss, pred = engine.eval_step(criterion, lr_img, gt_img)
            val_losses.append(loss)
            val_psnrs.append(compute_psnr(pred, gt_img.to(engine.device)))
        avg_val_loss = sum(val_losses) / len(val_losses)
        avg_val_psnr = sum(val_psnrs) / len(val_psnrs)

        old_lr = optimizer.param_groups[0]['lr']
        scheduler.step(avg_val_loss)
        new_lr = optimizer.param_groups[0]['lr']

        dt = time.time() - t0
        lr_msg = (f" [LR: {old_lr:.2e}]" if old_lr == new_lr
                  else f" [LR REDUCED: {old_lr:.2e} -> {new_lr:.2e}]")
        print(f"[SYNTH-AUG] Epoch {epoch+1}/{config['num_epochs']} "
              f"[{dt:.1f}s]{lr_msg} train_loss={avg_train_loss:.4f} "
              f"val_loss={avg_val_loss:.4f} val_psnr={avg_val_psnr:.2f}dB")

        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            epochs_since_improvement = 0
            ckpt_path = os.path.join(config["checkpoint_dir"], "best_model.pt")
            state_dict = (engine.model.module.state_dict()
                          if isinstance(engine.model, torch.nn.DataParallel)
                          else engine.model.state_dict())
            torch.save({"model_state": state_dict, "config": config,
                        "epoch": epoch, "val_loss": avg_val_loss,
                        "val_psnr": avg_val_psnr, "lr": new_lr,
                        "p_low": p_low, "p_high": p_high}, ckpt_path)
            print(f"  -> new best val_loss, saved to {ckpt_path}")
        else:
            epochs_since_improvement += 1
            print(f"  -> val_loss did not improve "
                  f"({epochs_since_improvement}/{config['early_stop_patience']})")
            if epochs_since_improvement >= config["early_stop_patience"]:
                if new_lr < config["lr"]:
                    print("STOPPING: model has fine-tuned at a lower LR and plateaued.")
                    break
                else:
                    print("WAITING: val_loss stalled but LR hasn't dropped yet.")

    engine.flush(optimizer)
    print("Synthetic-augmentation run finished. Compare val_psnr AND "
          "val_psnr_std against baseline (29.37dB, std 4.55) using "
          "evaluate.py -- the hoped-for effect is better generalization/"
          "robustness (steadier scores, better worst-case), not "
          "necessarily a higher mean.")


if __name__ == "__main__":
    main()