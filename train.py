...
def main():
    ...
    # ---- Config ----
    # NOTE: batch_size is now the single source of truth for how many
    # samples one train_step consumes (per DataLoader batch, across all
    # GPUs combined under DataParallel). accum_steps is derived from
    # target_batch_size // batch_size inside HardwareEngine -- there's no
    # separate local_batch_size to keep in sync anymore.
    config = dict(
        dim=64,
        num_blocks=3,
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for lr_img, gt_img in train_loader:
            # No `i` argument anymore -- accumulation position is tracked
            # internally by the engine and persists across epochs, so a
            # partial group at this epoch's end correctly carries into the
            # next epoch instead of being dropped.
    ...
    # CHANGE: flush any leftover accumulated gradient from a partial final
    # group at the true end of training (or an early-stop break above) --
    # otherwise up to accum_steps-1 microbatches' worth of gradient at the
    # very end of the run is silently discarded.
